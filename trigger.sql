...
BEGIN
    ...
    WHERE phoneModelID = (SELECT phoneModelID
    FROM phone
...
ROW
BEGIN
    DECLARE discountPercent DECIMAL
    (4,2);
...
SET NEW
.finalPrice = fixedNewPrice;
    ELSEIF discountPercent IS NOT NULL THEN
SET NEW
.finalPrice = NEW.originalPrice -
(NEW.originalPrice * discountPercent / 100);
    ELSEIF discountFixed IS NOT NULL THEN
SET NEW
.finalPrice = NEW.originalPrice - discountFixed;
    ELSE
SET NEW
.finalPrice = NEW.originalPrice;
-- Không có giảm giá
